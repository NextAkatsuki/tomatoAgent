...
## 12-29 해야할거
    - createChatName에서 질문과대답에서 대답부분이 안가는 버그가있음(react) 수정해야함
    - 백: gptArchive로 대화한 다음 gptArchive에 갔다오면 대화가 사라지는 버그가 있음 (react의 window모듈과 관련이 있을것으로 보임)
    - 백: 로그아웃할 때 토큰으로 관련된 redis값들 모두 지우는 작업 추가해야함
    - 백: Action, Observation 처럼 프롬프트에서 지정한 포맷대로 출력되지 않아 에러가 걸리는 상황이 있음 